...
import static com.reprezen.kaizen.normalizer.Option.OptionType.ADDITIONAL_FILES;
...
import com.reprezen.kaizen.normalizer.Option.ExtensionData;
import com.reprezen.kaizen.normalizer.Option.HoistType;
...
public class Options {
	...
	public boolean isInlined(Component type) {
		return optionDataHasValue(INLINE, type);
	}

	public boolean isRetained(Component component, boolean hasPaths) {
		if (options.get(RETAIN) == Option.PATH_OR_COMPONENTS) {
			return hasPaths == (component == V2State.PATH || component == V3State.PATH);
		} else {
			return optionDataHasValue(RETAIN, component);
		}
	}
	...
	public List<URL> getAdditionalFileUrls() {
		...
		for (String urlString : getAdditionalFileStrings()) {
			results.add(new Reference(fixFile(urlString), topRef).getUrl());
		}
		return results;
	}
	...
	public void resolveScope(String topUrlString) throws MalformedURLException {
		...
		topRef = new Reference(topUrlString);
	...
	public String toString() {
		...
		return "[" + opts.stream().collect(Collectors.joining("\n")) + "]";
	}
}